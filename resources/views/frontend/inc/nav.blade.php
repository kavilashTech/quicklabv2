...
<div class="top-navbar border-bottom border-soft-secondary z-1035 h-35px h-sm-auto" style="background-color:#243824;">
            ...
            <div class="col-lg-4 col">
                <ul class="list-inline d-flex justify-content-between justify-content-lg-start mb-0">
                 ...
                 <li class="list-inline-item dropdown mr-3" style="color:#ffffff!important" id="lang-change">
                   ...
                   <li class="list-inline-item dropdown ml-auto ml-lg-0 mr-0" style="color:#ffffff!important" id="currency-change">
                        @php
  ...
  @if (auth()->user() != null)  {{ Auth::user()->name }}  @else  Sign In @endif
  ...
  <div class="dropdown-menu list-inline-item mr-3 border-right border-left-0 pr-3 pl-0" aria-labelledby="dropdownMenuButton">
     @if (auth()->user() != null)
        ...
        @else
            <a href="{{ route('dashboard') }}" class="dropdown-item  @if(!empty(Auth::user()) && Auth::user()->user_type == 'partner' && (Auth::user()->status == 0 || Auth::user()->status == 3)) disabled @endif" ><i class="las la la-dashboard aiz-side-nav-icon"></i><span class="aiz-side-nav-text">{{ translate('My Panel')}}</span></a>
        @endif
    @else
                    ...
                    @auth
                        ...
                        @else
                            ...
                            <li class="list-inline-item mr-3 border-right border-left-0 pr-3 pl-0 dropdown">
                                <a class="dropdown-toggle no-arrow text-reset @if(!empty(Auth::user()) && Auth::user()->user_type == 'partner' && (Auth::user()->status == 0 || Auth::user()->status == 3)) disabled @endif" data-toggle="dropdown" href="javascript:void(0);" role="button" aria-haspopup="false" aria-expanded="false">
                                    <span class="">
        ...
        <div class="container">
            <div class="d-flex align-items-center">
                ...
                <div class="col-auto col-xl-3 pl-0 pr-3 d-flex align-items-center">
                    <a class="d-block py-20px mr-3 ml-0 @if(!empty(Auth::user()) && Auth::user()->user_type == 'partner' && (Auth::user()->status == 0 || Auth::user()->status == 3)) disabled @endif" href="{{ route('home') }}">
                        @php
                    ...
                    <div class="position-relative flex-grow-1">
                        <form action="{{ route('search') }}" method="GET" class="stop-propagation">
                            <div class="d-flex position-relative align-items-center">
                                ...
                                <div style="width:50%;">
                                    <select class="form-control"  name="category" @if(!empty(Auth::user()) && Auth::user()->user_type == 'partner' && (Auth::user()->status == 0 || Auth::user()->status == 3)) disabled @endif>
                                ...
                                <div class="input-group">
                                    <input type="text" class="border-0 border-lg form-control" id="search" name="keyword" @isset($query)
                                        value="{{ $query }}"
                                    @endisset placeholder="{{translate('Search Products by title, supplier, sku, category, brand etc...')}}" autocomplete="off" @if(!empty(Auth::user()) && Auth::user()->user_type == 'partner' && (Auth::user()->status == 0 || Auth::user()->status == 3)) disabled @endif>
                                    <div class="input-group-append d-none d-lg-block">
                                        <button class="btn btn-primary" type="submit" @if(!empty(Auth::user()) && Auth::user()->user_type == 'partner' && (Auth::user()->status == 0 || Auth::user()->status == 3)) disabled @endif>
            ...
            <div class="container">
                <ul class="list-inline mb-0 pl-0 mobile-hor-swipe text-center">
                    ...
                    <li class="list-inline-item mr-0">
                        <a href="{{ env('APP_URL') }}{{ json_decode( get_setting('header_menu_links'), true)[$key] }}" class="opacity-60 fs-14 px-3 py-2 d-inline-block fw-600 hov-opacity-100 text-reset @if(!empty(Auth::user()) && Auth::user()->user_type == 'partner' && (Auth::user()->status == 0 || Auth::user()->status == 3)) disabled @endif">
                            {{ translate($value) }}
                        </a>
                    </li>
                    @endforeach
