...
class LoginController extends Controller
{
    ...
    */
    ...
    public function authenticated()
    {
        ...
        // else if (auth()->user()->user_type == 'partner' && auth()->user()->user_type != 'customer' && (empty(auth()->user()->id_proof) || empty(auth()->user()->address_proof))) {
        //     $data = array('email' => auth()->user()->email);
        //     auth()->logout();
        //     return view('frontend.franchisee_uploads_documents',compact('data'));

        // }
        else if (auth()->user()->user_type == 'partner' && auth()->user()->status == 0  && auth()->user()->email_verified_at == '' ) {
            $redirect_route = 'profile';
            $this->guard()->logout();

            flash(translate(' please Verify Your Email Address'))->error();
            return back();


        }
        else if (auth()->user()->user_type == 'partner' && auth()->user()->status == 3) {
            return redirect()->route('franchisee-registration-form');
        }
        else if (auth()->user()->user_type == 'partner' && auth()->user()->status == 0  ) {
           
            //TODO: Hide for ticket (0000283)
            /*$redirect_route = 'home';
            $this->guard()->logout();
            flash(translate('Your email has been verified successfully please wait for admin Approval'))->error();
            return back();*/

            return redirect()->route('franchisee-registration-form');

        }
        else if (auth()->user()->user_type == 'partner' && auth()->user()->status == 2  ) {
            $redirect_route = 'home';
            $this->guard()->logout();

            flash(translate('Your profile has been Rejected please check your email for rejected reason'))->error();
            return back();


        } else if (auth()->user()->user_type == 'customer' && auth()->user()->email_verified_at == '') {
            //NEW
            ...
            $redirect_route = 'home';
            $this->guard()->logout();
            flash(translate('Email verification Pending. Please verify.'))->error();
            return back();
